...
string DataSave3D::name() const
{
	return "数据保存3D......";
}

bool DataSave3D::execute(CTData3D& ctdata)
{ 
...
bool DataSave3D::saveFloatArrayToRaw(const float* data, size_t elem_count, const fs::path& save_path) {
    // 空数据/空路径检查
    if (data == nullptr) {
